#!/bin/bash
# Launch an MCP server module from the hokusai_mcp package.
# Usage: run.sh <module>   e.g. run.sh hokusai_mcp.hpc_server
set -euo pipefail
...
if ! "$VENV/bin/python" -c "import hokusai_mcp, hpc_agent_core.mcp_server" >/dev/null 2>&1; then
