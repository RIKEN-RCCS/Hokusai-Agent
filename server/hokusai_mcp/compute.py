...
from hokusai_mcp.middleware import run_command, write_remote_file
from hokusai_mcp.models import Job, JobSpec, JobState, JobStatus, map_slurm_state
...
def render_script(spec: JobSpec) -> str:
    """Render a JobSpec as an HBW2 sbatch script."""
    res = spec.resources
    ...
    # GPUs are optional on HBW2 (GPU server only). `gpus` (HBW2 extension →
    # --gpus) takes precedence over the PSI/J gpu_cores_per_process.
    ...
    # HBW2 requires --account for job execution; fall back to the config default.
    ...
    if spec.container:
        # HBW2 provides Singularity; wrap the command in `singularity exec`.
        c = spec.container
        sing_flags = []
...
def submit(spec: JobSpec) -> dict:
    ...
    script_path = write_remote_file(
        f".hokusai/jobs/{spec.name}-{stamp}.sh", render_script(spec)
    )
