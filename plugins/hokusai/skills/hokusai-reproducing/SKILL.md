...
hpc = connect_sync(
    pinned_params(
        "https://github.com/RIKEN-RCCS/Hokusai-Agent.git",
        "hokusai-hpc-mcp",
        ref="main",
        subdirectory="server",
    ),
...
5. **If a partition choice isn't given or obvious, check `get_resources`**
   and pick from that — don't guess a default (`mpc`, `mpc_l`, and other
   HBW2 partitions differ in node count and time limit).
